...
えっボイラープレートを書かなくても設定管理ができる？再現性のあるスクリプトが書ける？NotebookをベースにCLIでハイパラサーチできる？勝手にフォルダ分けしてくれる？魔法のDeveloper ExperienceでGPUぶんまわそう！
...
- 🤖 **CLIからハイパラチューニング。** コマンドラインから直接ハイパーパラメータを繰り返し調整可能。
...
ご意見がありましたら、info@haipera.comまでお知らせください。また、解決したい切実な問題やニーズがあれば、ぜひお聞かせください！
...
Pythonで色々と実験してる時、次のようなスクリプトを書くことがあります：
...
同じフォルダに、依存関係をリストアップしたrequirements.txtがあるかもしれません：

```
numpy
```

これはあまり現実的ではない例ですが、このコードで実験を始めたいとします。

最初は`num_apples`と`apple_price`を手動で調整するでしょうが、変数が増えてくると変更を追跡できなくなります。

適切にこれらの変数を追跡するには、これらの変数をコマンドラインインターフェースから編集できるようにしたり、ノートブックをセットアップしたり、これを追跡するためのJSONやYAMLファイルを設定したり、ログサービスに出力をログしたり、別々の実験フォルダに出力や設定を保存するなどが必要です。
...
これは、ノートブックのバージョン管理を行う便利な方法となります。異なるデータや異なる例に使用したいノートブックがある場合、同じノートブックの8つのクローンを作成する代わりに、単一のノートブックと8つの異なる設定ファイルを用意するだけで済みます！

Pythonスクリプトをノートブックとして実行することもできますが、通常それを行う大きな理由はないでしょう。
...
Haiperaはまだ初期段階にあるため、バグがある可能性が高いです。私たちは積極的にHaiperaを開発しているので、GitHubでイシューを立てるか、Discordサーバーでコメントするか、support@haipera.comまでメールを送っていただければ、できるだけ早く解決するよう努めます！
